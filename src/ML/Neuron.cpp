...
void Neuron::setValue(double val) { _value = val; }
...
void Neuron::_weightsGrad() {
  for (auto n : _weightsGradients | std::ranges::views::keys) {
    double nextGradient =
        _gradient.value() * _activation.diff(_aggregate.value()) * n->eval();
    if (_weightsGradients[n].first.has_value()) {
      _weightsGradients[n].first =
          (_weightsGradients[n].second * _weightsGradients[n].first.value() +
           nextGradient) /
...
double Neuron::grad() {
  ...
  for (const auto n : _outputs)
    g += n->grad() * n->_inputs[this] *
         n->_activation.diff(n->_aggregate.value());
  _gradient = g;
...
void Neuron::updateWeights(const double learningRate) {
  // Modify each weight according to the computed mean partial derivative with
  // respect to an optimizer (TODO)
...
std::map<Neuron *, double> &Neuron::inputs() { return _inputs; }

void Neuron::draw(const float r) const {
  for (Neuron *key : _inputs | std::views::keys) {
    const float weight =
        Clamp(static_cast<float>(_inputs.at(key)), -1.0f, 1.0f);
    const Color c = Utils::Colors::UniformGradient(weight, -1.0f, 1.0f,
                                                   {RED, DARKGRAY, GREEN});
    DrawLine(static_cast<int>(position.x), static_cast<int>(position.y),
             static_cast<int>(key->position.x),
             static_cast<int>(key->position.y), c);
    // DrawLineBezier(position, key->position, 1.0f, c);
  }
  // DrawCircle(static_cast<int>(position.x), static_cast<int>(position.y), r,
  // WHITE);
  DrawRectangle(static_cast<int>(position.x - r),
                static_cast<int>(position.y - r), 2 * r, 2 * r, WHITE);

  const std::string txt = fmt::format("{:.3}", _value.value_or(0));
  DrawText(
      txt.c_str(),
      static_cast<int>(position.x -
                       MeasureText(txt.c_str(), static_cast<int>(r)) / 2.0),
      static_cast<int>(position.y - r / 2.0), static_cast<int>(r), GRAY);
}

Vector2 Neuron::computeDrawSize(const float r) { return {2 * r, 2 * r}; }

// Activation functions
double reluEval(const double x) { return std::max(0.0, x); }
double reluDiff(const double x) { return x <= 0 ? 0.0 : 1.0; }
double identityEval(const double x) { return x; }
double identityDiff(const double x) { return 1.0; }
double sigmoidEval(const double x) { return 1.0 / (1 + std::exp(-x)); }
...
const ActivationFunc &Neuron::ReLu() { return reluStruct; }
const ActivationFunc &Neuron::Identity() { return identityStruct; }
const ActivationFunc &Neuron::Sigmoid() { return sigmoidStruct; }
