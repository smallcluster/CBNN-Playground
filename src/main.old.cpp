...
const ML::ActivationFunc &getActivationFunction(const int index) {
  switch (index) {
  case 1:
    return ML::Neuron::Sigmoid();
  case 2:
    return ML::Neuron::Identity();
  default:
    return ML::Neuron::ReLu();
  }
}
...
ML::MLP buildCBNR(const std::vector<int> &layerSizes,
  ...
  for (int i = 0; i < layerSizes.size(); ++i)
    model.addLayer(layerSizes[i], getActivationFunction(activationFunctions[i]),
                   true);
...
Color normalizedToColor(const std::vector<double> &values) {
  return {static_cast<unsigned char>(std::clamp(values[0], 0.0, 1.0) * 255),
...
std::vector<double> colorToNormalized(const Color &color) {
  return {static_cast<double>(color.r) / 255.0,
          static_cast<double>(color.g) / 255.0,
          static_cast<double>(color.b) / 255.0};
}

std::vector<Color> predictColors(const ML::MLP &model, const int width,
                                 const int height) {
...
int main(int argc, char *argv[]) {
  ...
  const std::vector<const char *> activationFuncChoices = {"ReLu", "Sigmoid",
                                                           "Identity"};
  ...
  const auto stepTrain =
      [&](const std::function<void(const std::vector<double> &trueValues)>
              &gradFunc,
        ...
        postGradFunc();
        // Record image MSE
        const double imageMSE =
            std::accumulate(mseValues.begin(), mseValues.end(), 0.0) / (w * h);
  ...
  const auto stepTrainSGD = [&] {
    stepTrain(
        [&](const std::vector<double> &trueValues) {
  ...
  while (!WindowShouldClose()) {
    ...
    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
      panOrigin = {camera.target.x * camera.zoom +
                       (static_cast<float>(GetMouseX()) - camera.offset.x),
                   camera.target.y * camera.zoom +
                       (static_cast<float>(GetMouseY()) - camera.offset.y)};
    }
    if (IsMouseButtonReleased(MOUSE_BUTTON_LEFT)) {
      panOrigin = {};
    }
    if (panOrigin.has_value()) {
      camera.target.x = -(static_cast<float>(GetMouseX()) - camera.offset.x -
                          panOrigin.value().x) /
                        camera.zoom;
      camera.target.y = -(static_cast<float>(GetMouseY()) - camera.offset.y -
                          panOrigin.value().y) /
                        camera.zoom;
    }
    ...
    if (IsFileDropped()) {
      ...
      Image imageOutput =
          GenImageColor(imageInput->width, imageInput->height, BLACK);
    ...
    if (animatedEval) {
      ...
      auto values = model.eval({static_cast<double>(currentX) / w,
                                static_cast<double>(currentY) / h});
      ...
      const float factor = 1.0f - static_cast<float>(evalProgress) /
                                      static_cast<float>(w * h - 1);
      const float barWidth = static_cast<float>(GetScreenWidth()) * factor;
      BeginScissorMode(0, GetScreenHeight() - 16, static_cast<int>(barWidth),
                       16);
      DrawRectangleGradientH(0, GetScreenHeight() - 16, GetScreenWidth(), 16,
                             RED, GREEN);
      EndScissorMode();
    ...
    // model drawing
    BeginMode2D(camera);
    const Vector2 modelDrawSize =
        model.computeDrawSize(neuronRadius, layerPadding, neuronPadding);
    model.draw(modelDrawSize * -1 / 2.0f, neuronRadius, layerPadding,
               neuronPadding);
    EndMode2D();
    ...
    if (ImGui::Begin("Model performance")) {
      if (!avgMSE.empty()) {
        ImGui::Text(fmt::format("Latest MSE: {:.6}", avgMSE[avgMSE.size() - 1])
                        .c_str());
        ImGui::Text(fmt::format("Total training steps: {}",
                                static_cast<int>(avgMSE.size()))
                        .c_str());
        if (training)
        ...
        const int toCopy =
            std::min(static_cast<int>(avgMSE.size()), mseViewSize);
        mseView.reserve(toCopy);

        for (int i = 0; i < toCopy; ++i)
          mseView.push_back(
              avgMSE[static_cast<int>(avgMSE.size()) - toCopy - 1 + i]);

        ImPlot::PlotLine("MSE##data", mseView.data(),
                         static_cast<int>(mseView.size()));
        ImPlot::EndPlot();
      } else
    ...
    if (imageInput.has_value()) {
      ...
      if (ImGui::Combo("Training method", &selectedTrainMethod,
                       trainMethod.data(),
    ...
    for (int i = 0; i < nbDeepLayers; i++) {
      if (ImGui::CollapsingHeader(fmt::format("Layer {} Size", i).c_str())) {
        ...
        ImGui::Combo(fmt::format("Activation function ##Layer {}", i).c_str(),
                     &deepLayerActivationFuncs[i], activationFuncChoices.data(),
                     static_cast<int>(activationFuncChoices.size()));
    ...
    if (textureInput.has_value()) {
      ImGui::Text(
          fmt::format("Size: {}x{}", textureInput->width, textureInput->height)
              .c_str());
